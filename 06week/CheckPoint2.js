...
const nums = strNums.map( (piDigit) => {
  return Number(piDigit);
});


// Find the sum of the even values
const sumEvens = nums.reduce( (accumulator, currentValue) => {
  if(currentValue % 2 == 0){
...
const atxIdx = nums.findIndex( (piDigit,index) => {
  if( piDigit + index == 512 ){
    return index;
  }
});
